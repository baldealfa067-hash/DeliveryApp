-- FASE 1 (fecho) -- alinhar o texto das notificacoes com o rotulo "Concluido".
--
-- `entregue` foi fundido em `concluido` em 20260909155503, mas o texto que o
-- cliente recebe continuava a dizer "Pedido entregue. Bom apetite!". O estado
-- passou a chamar-se uma coisa e a notificacao dizia outra.
--
-- AMBITO: so' texto voltado ao utilizador. Nenhum nome de estado muda
-- (Aditamento 2), nenhuma logica de transicao muda. Duas funcoes escreviam
-- este texto:
--   update_order_status  -- v_status_label do ramo 'concluido'
--   complete_delivery    -- title + body da notificacao final
--
-- As duas sao recriadas a partir da sua ultima versao (20260909155602 e
-- 20260909155641), com a unica diferenca a ser a cadeia de texto.
...
BEGIN
  ...
  IF v_customer_id IS NOT NULL THEN
    INSERT INTO public.notifications (user_id, type, title, body, link)
    VALUES (v_customer_id, 'order_update', 'Pedido concluido',
            'Pedido concluido. Bom apetite!', '/meus-pedidos');
  END IF;
END;
$$;

-- NOTA: `complete_delivery` emitia '✅ Pedido entregue...' e `update_order_status`
-- emitia 'Pedido entregue...' sem emoji -- o mesmo acontecimento com dois textos
-- diferentes conforme o caminho. Ficam agora identicos, sem emoji.
