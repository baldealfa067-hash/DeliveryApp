-- ============================================================
-- Extensão de pedidos: Bairro + Estado "Saiu para entrega"
-- ============================================================

-- 1. Adicionar coluna bairro à tabela orders
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS bairro text;

-- 2. Adicionar 'saiu_para_entrega' ao CHECK de status
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_status_check;
ALTER TABLE public.orders ADD CONSTRAINT orders_status_check
  CHECK (status IN (
    'novo', 'confirmado', 'em_preparacao', 'na_cozinha', 'pronto',
    'saiu_para_entrega',
    'aguardando_motorista', 'motorista_encontrado', 'pedido_recolhido',
    'a_caminho', 'entregue', 'concluido', 'cancelado'
  ));

-- 3. Atualizar create_order para aceitar p_bairro
...
REVOKE ALL ON FUNCTION public.create_order(uuid, uuid, text, text, jsonb, numeric, text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_order(uuid, uuid, text, text, jsonb, numeric, text, text, text, text) TO authenticated;

-- 4. Atualizar update_order_status para incluir 'saiu_para_entrega' e notificação
...
BEGIN
  ...
  SELECT o.id, o.order_number, o.customer_name, o.customer_phone,
         ...
         o.created_at, o.updated_at, o.bairro
