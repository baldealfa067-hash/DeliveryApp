-- Fix: when delivery order is marked "pronto", record "pronto" in history
-- BEFORE auto-transitioning to "aguardando_motorista".
-- Previously only v_final_status was recorded, so "pronto" never appeared
-- in the client timeline for delivery orders.
...
BEGIN
  ...
  IF p_new_status = 'pronto' AND v_consumption = 'entrega' THEN
    IF NOT EXISTS (SELECT 1 FROM public.deliveries WHERE order_id = p_order_id) THEN
      ...
      IF v_restaurant_lat IS NOT NULL AND v_restaurant_lng IS NOT NULL
        ...
        -- Record "pronto" in history BEFORE overriding to aguardando_motorista
  ...
  -- Record v_final_status in history (pronto already recorded above if auto-transitioned)
  INSERT INTO public.order_status_history (order_id, status, note, created_by)
