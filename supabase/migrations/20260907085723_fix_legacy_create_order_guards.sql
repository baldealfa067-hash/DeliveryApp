-- Versoes legadas de create_order (9/10/12 args): ja nao estavam abertas ao anon,
-- mas permitiam a um utilizador autenticado criar pedidos em nome de outro.
-- Recebem a mesma verificacao; o corpo mantem-se igual.

CREATE OR REPLACE FUNCTION public.create_order(p_business_id uuid, p_customer_id uuid, p_customer_name text, p_customer_phone text, p_items jsonb, p_total numeric, p_consumption_option text, p_address text DEFAULT NULL::text, p_notes text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.create_order(p_business_id uuid, p_customer_id uuid, p_customer_name text, p_customer_phone text, p_items jsonb, p_total numeric, p_consumption_option text, p_address text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_bairro text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.create_order(p_business_id uuid, p_customer_id uuid, p_customer_name text, p_customer_phone text, p_items jsonb, p_total numeric, p_consumption_option text, p_address text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_bairro text DEFAULT NULL::text, p_customer_lat double precision DEFAULT NULL::double precision, p_customer_lng double precision DEFAULT NULL::double precision)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
